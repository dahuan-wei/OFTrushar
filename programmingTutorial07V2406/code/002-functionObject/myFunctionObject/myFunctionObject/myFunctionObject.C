...
{
    sumPatchmyFunctionObjectP_ = Zero;
    sumPatchmyFunctionObjectV_ = Zero;
    sumPatchMomentsP_ = Zero;
    sumPatchMomentsV_ = Zero;

    sumInternalmyFunctionObject_ = Zero;
...
Foam::functionObjects::myFunctionObject::devRhoReff
(
...
)
{
    ...
    sumPatchmyFunctionObjectP_ += sum(fP);
    sumPatchmyFunctionObjectV_ += sum(fV);
...
)
{
    ...
    forAll(cellIDs, i)
    {
        ...
        sumInternalmyFunctionObject_ += f[i];
        force[celli] += f[i];
...
void Foam::functionObjects::myFunctionObject::writeIntegratedDataFiles()
{
    ...
    (
        coordSys.localVector(sumPatchmyFunctionObjectP_),
        coordSys.localVector(sumPatchmyFunctionObjectV_),
        coordSys.localVector(sumInternalmyFunctionObject_),
        forceFilePtr_()
    );
...
void Foam::functionObjects::myFunctionObject::calcmyFunctionObjectMoments()
{
    initialise();
    ...
    reduce(sumPatchmyFunctionObjectP_, sumOp<vector>());
    reduce(sumPatchmyFunctionObjectV_, sumOp<vector>());
    reduce(sumPatchMomentsP_, sumOp<vector>());
    reduce(sumPatchMomentsV_, sumOp<vector>());
    reduce(sumInternalmyFunctionObject_, sumOp<vector>());
...
{
    return sumPatchmyFunctionObjectP_ + sumPatchmyFunctionObjectV_ + sumInternalmyFunctionObject_;
}
...
{
    calcmyFunctionObjectMoments();

    Log << type() << " " << name() << " write:" << nl;

    const auto& coordSys = coordSysPtr_();

    const auto localFp(coordSys.localVector(sumPatchmyFunctionObjectP_));
    const auto localFv(coordSys.localVector(sumPatchmyFunctionObjectV_));
    const auto localFi(coordSys.localVector(sumInternalmyFunctionObject_));
